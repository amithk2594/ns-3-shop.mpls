...
public:
   ...
   */
  static const Label& GetIpv4ExplicitNull (void);
  /**
   * @brief Get IPv6 explicit null label
   */
  static const Label& GetIpv6ExplicitNull (void);
  /**
   * @brief Get route alert label
   * @return Route alert label
   */
  static const Label& GetRouteAlert (void);
  /**
   * @brief Get null label
   * @return Implicit null label
   */
  static const Label& GetImplicitNull (void);

private:
  Label (uint32_t value, uint32_t t);
  uint32_t m_value;
 ...
 */
inline uint32_t Get (Label label, uint8_t ttl)
{
  return (uint32_t(label) << 12) | ttl;
}

/**
 * @brief Calculates and returns the shim
 */  
inline uint32_t Get (Label label, uint8_t ttl, uint8_t exp)
{
  return (uint32_t(label) << 12) | ttl | (exp << 9);
}
 ...
 */
inline uint32_t SetTtl (uint32_t s, uint8_t ttl)
{
